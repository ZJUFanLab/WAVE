    ...
    with h5py.File(fname, 'r') as f:
        for key in f:
            ...
            if isinstance(data[key][0], np.bytes_):
                data[key] = data[key].astype(np.str_)
    return data
...
outdir = "/slurm/home/yrd/liaolab/lvtianhang/Work/WAVE/04.benchmark/cell_unseen_v2/TranSiGen"
...
for Fold in Folds:
    f = os.path.join(outdir, Fold, "results", "trained_models_9_cell_cells_split",
                     Fold[-1], "feature_KPGT_init_initial_None", "predict",
                     "test_prediction_profile.h5")
    data = load_from_HDF(f)

    truth = data['x2']
    pred  = data['x2_pred']
    ctrl  = data['x1']
    cell_id = data['cid'][0].astype(str).tolist()
