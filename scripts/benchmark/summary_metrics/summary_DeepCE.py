...
def calc_pearson(preds, targets):
    return np.array([pearsonr(p, t)[0] for p, t in zip(preds, targets)])
...
def compute_pearson_top50(true_expr, gene_expr, output):
    ...
    for i in range(true_expr.shape[0]):
        ...
        pearson = pearsonr(true_expr[i, top50_indices], output[i, top50_indices])[0]
...
def compute_precision_recall_at_k(delta_pred, delta_truth, k=100):
    """
    Per-sample positive/negative precision@k and recall@k.

    Args:
        delta_pred:  (N, G) predicted expression change
        delta_truth: (N, G) true expression change
        k: number of top genes

    Returns:
        dict of arrays, each (N,)
    """
    ...
    for i in range(n_samples):
        # Positive (up-regulated): largest delta values
        truth_pos = set(np.argsort(delta_truth[i])[-k:])
        pred_pos  = set(np.argsort(delta_pred[i])[-k:])

        # Negative (down-regulated): smallest delta values
...
adata = sc.read_h5ad("/path/to/test_L1000_level4_benchmark.h5ad")

ctrl = adata.layers['unpert_expr']
truth = adata.X

outdir = "/path/to/DeepCE"
...
for Fold in Folds:
    pred_f = os.path.join(outdir, Fold, "prediction.txt")
    ...
    # Fold means
    fold_means['RMSE'].append(np.mean(rmse))
    fold_means['SSE'].append(np.mean(sse))
    fold_means['MSE'].append(np.mean(mse))
    fold_means['MAE'].append(np.mean(mae))
    fold_means['R2'].append(np.mean(r2))
    fold_means['Pearson'].append(np.mean(pearson))
    fold_means['Delta_x_pearson'].append(np.mean(delta_p))
    fold_means['top50'].append(np.mean(top50))
    for key in pk_results:
        fold_means[key].append(np.mean(pk_results[key]))
...
for metric, values in fold_means.items():
    results.append({
        ...
        'Std':  f"{np.std(values):.5f}"
    })

results_df = pd.DataFrame(results)
print(results_df)
results_df.to_csv("DeepCE_metrics_sum.csv", index=False)

# ===== Per-sample table =====
metrics_df = pd.DataFrame(all_samples)
metrics_df.to_csv("DeepCE_metrics.csv", index=False)

print(f"\nSaved DeepCE_metrics_sum.csv and DeepCE_metrics.csv")
...
# Flatten MultiIndex columns: e.g. ('RMSE', 'mean') -> 'RMSE_mean'
cell_summary.columns = [f"{col}_{stat}" for col, stat in cell_summary.columns]
cell_summary = cell_summary.reset_index()

cell_summary.to_csv("DeepCE_metrics_per_cell.csv", index=False)
print(f"\nSaved DeepCE_metrics_per_cell.csv ({len(cell_summary)} cell lines)")
